...
class MiniSom(object):
    ...
    def correctweights(self,x,win, t, max_iteration):
        ...
        semipart=int((g.shape[1]-1)/2)
        np.seterr(all='ignore')
        x[semipart]+=np.mean(np.einsum("ij,jk->ik",g,x),axis=0)
        return x[semipart]


    def update(self, x, t, max_iteration):
        """Updates the weights of the neurons.
